       ...
       /* case v_HGCAL_2016TB:
        {
            ...
            //
            for (int i = 0; i < 3; i++) {
                lThick.push_back(4.2 * mm);
                lEle.push_back("W");
                lThick.push_back(absPlateAirGap);
                lEle.push_back("Air");
            }
            for (int i = 0; i < 2; i++) {
                lThick.push_back(2.1 * mm);
                lEle.push_back("W");
                lThick.push_back(absPlateAirGap);
                lEle.push_back("Air");
            }
            lThick.push_back(2.1 * mm);
            lEle.push_back("W");
            lThick.push_back(3.0 * mm);
            lEle.push_back("Air");

            lThick.push_back(0.6 * mm);
            lEle.push_back("WCu");
            lThick.push_back(6.0 * mm);
            lEle.push_back("Cu");
            lThick.push_back(0.6 * mm);
            lEle.push_back("WCu");
            lThick.push_back(0.01 * mm);
            lEle.push_back("Air"); // kapton
            lThick.push_back(0.1 * mm);
            lEle.push_back("Si");
            lThick.push_back(0.1 * mm);
            lEle.push_back("Si");
            lThick.push_back(0.12 * mm);
            lEle.push_back("Si");
...
/*void DetectorConstruction::SetWThick(std::string thick)
{
  ...
  for (unsigned iE(0); iE<vec.size(); ++iE){
...
/*void DetectorConstruction::SetDropLayers(std::string layers)
{
  ...
  for (unsigned iE(0); iE<dropLayer_.size(); ++iE){
...
G4VSolid *DetectorConstruction::constructSolid (std::string baseName, G4double thick, G4double zpos,const G4double & minL, const G4double & width){
  ...
  if (model_ == DetectorConstruction::m_FULLSECTION){
    ...
    double maxR = /*tan(2*atan(exp(-m_minEta)))*(zpos+m_z0pos+m_CalorSizeZ/2);*/tan(m_maxTheta) * (zpos + m_z0pos + m_CalorSizeZ / 2);
